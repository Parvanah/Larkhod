...
const SingUp = ({ t }, props) => {
  ...
  // const [register] = useContext(AuthContext);
    const [googleUseInfo, setGoogleUserInfo] = useState(null);
  ...
  const login = useGoogleLogin({
    onSuccess: async (respose) => {
      try {
        ...
        GoogleLoginInformation(setGoogleUserInfo);
      //  navigate("/home");
...
const GoogleLoginInformation = (setGoogleUserInfo) =>{
  ...
  const [googleUserInfo, setgoogleUserInfo ] = useState(setGoogleUserInfo)
  ...
  const handleChange = (e) => {
    const { name, value, googlevalue } = e.target;
    setFormValues({ ...formValues, [name]: value });
    setgoogleUserInfo({ ...googleUseInfo, [name]: googlevalue });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    setFormErrors(validate(formValues, googleUserInfo));
    console.log(
  ...
  useEffect(() => {
    ...
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      console.log(googleUseInfo);
    }
  }, [formErrors]);
  const validate = (values, googlevalue) => {
    const errors = {};
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i;
    if (!googlevalue.firstname) {
      errors.firstname = <>{t("singup22")}</>;
    }
    if (!googlevalue.lastname) {
    ...
    if (!googlevalue.email) {
      errors.email = <>{t("singup10")}</>;
    } else if (!regex.test(googlevalue.email)) {
